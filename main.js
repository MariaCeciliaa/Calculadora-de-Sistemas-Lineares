document.addEventListener('DOMContentLoaded', function () {
  ...
  function showHideGaussSeidelInputs() {
      var opcao = document.getElementById("opcao").value;
      var coeficientesDiv = document.getElementById("coeficientes");

      if (opcao === "2") {
          var ordemMatriz = parseInt(document.getElementById("numero-equacoes").value);
          var inputsHtml = "";

          for (var i = 0; i < ordemMatriz; i++) {
              inputsHtml += '<label>Chute inicial para x' + (i + 1) + ':</label>';
              inputsHtml += '<input type="number" id="chute-inicial-' + i + '"><br><br>';
          }

          coeficientesDiv.innerHTML = inputsHtml;
      } else {
          coeficientesDiv.innerHTML = "";
      }
  }

  document.getElementById("opcao").addEventListener("change", showHideGaussSeidelInputs);
  ...
  function calcularSistemaLinear() {
      ...
      if (opcao === 1) {
          ...
          var chuteInicial = [];
          for (var i = 0; i < numeroEquacoes; i++) {
              var input = document.getElementById("chute-inicial-" + i);
